...
{
  struct pointer_set_t *result = malloc (sizeof (struct pointer_set_t));

  result->n_elements = 0;
  result->log_slots = 8;
  result->n_slots = (size_t) 1 << result->log_slots;

  result->slots = calloc (sizeof (const void *), result->n_slots);
  return result;
}
  ...
  if (pset->n_elements > pset->n_slots / 4)
    {
      ...
      const void **new_slots = calloc (sizeof (const void *), new_n_slots);
      size_t i;
...
{
  struct pointer_map_t *result = malloc (sizeof (struct pointer_map_t));

  result->n_elements = 0;
  result->log_slots = 8;
  result->n_slots = (size_t) 1 << result->log_slots;

  result->keys = calloc (sizeof (const void *), result->n_slots);
  result->values = calloc (sizeof (void *), result->n_slots);
  return result;
}
  ...
  if (pmap->n_elements > pmap->n_slots / 4)
    {
      ...
      const void **new_keys = calloc (sizeof (const void *), new_n_slots);
      void **new_values = calloc (sizeof (void *), new_n_slots);
      size_t i;
