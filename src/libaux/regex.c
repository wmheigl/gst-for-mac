...
#define DOUBLE_STACK(type) do {						\
  ...
  if (stackb == stacka) {						\
    stackx = (type*)malloc(2 * xlen * sizeof(type));			\
    memcpy(stackx, stackb, xlen * sizeof (type));			\
  }									\
  else {								\
    stackx = (type*)realloc(stackb, 2 * xlen * sizeof(type));		\
  }									\
...
#define PRE_TALLOC(n,t)  ((t*)alloca((n)*sizeof(t)))
#define TMALLOC(n,t)    ((t*)malloc((n)*sizeof(t)))
#define TREALLOC(s,n,t) (s=((t*)realloc(s,(n)*sizeof(t))))

#define EXPAND_FAIL_STACK() DOUBLE_STACK(unsigned char*)
#define ENSURE_FAIL_STACK(n)						\
  do {									\
    if (stacke - stackp <= (n)) {					\
	/* if (len > pre_max_failures * MAX_NUM_FAILURE_ITEMS)		\
	   {								\
...
/* This must be nonzero for the wordchar and notwordchar pattern
   commands in pre_match.  */
#define Sword  1

#define SYNTAX(c) pre_syntax_table[c]
...
/* These are the command codes that appear in compiled regular
   ...
   So regex.h defines a symbol `PRE_EXACTN_VALUE' to be 1; the value of
...
enum regexpcode
{
  ...
  start_memory,			/* Start remembering the text that is matched, for
				   ...
				   must be in the range 0 through PRE_NREGS.  */
  stop_memory,			/* Stop remembering the text that is matched
				   and store it in a memory register.  Followed by
				   one byte containing the register number. Register
				   numbers must be in the range 0 through PRE_NREGS.  */
...
/* Macros for pre_compile_pattern, which is found below these definitions.  */

#define TRANSLATE_P() ((options&PRE_OPTION_IGNORECASE) && translate)
#define MAY_TRANSLATE() ((bufp->options&(PRE_OPTION_IGNORECASE|PRE_MAY_IGNORECASE)) && translate)
...
#define IS_IN_LIST(c, list) (						\
  ((unsigned char)(c) < ((unsigned) *(list)) * BYTEWIDTH) &&            \
    ((list)[1 + (unsigned char)(c) / BYTEWIDTH]				\
     & (1 << ((unsigned char)(c) % BYTEWIDTH))                          \
  ))
...
print_compiled_pattern (bufp)
     struct pre_pattern_buffer *bufp;
{
...
/* pre_compile_pattern takes a regular-expression string
   and converts it into a buffer full of byte commands for matching.

   PATTERN   is the address of the pattern string
   SIZE      is the length of it.
   BUFP	    is a  struct pre_pattern_buffer *  which points to the info
	     on where to store the byte commands.
	     This structure contains a  char *  which points to the
	     actual space, which should have been obtained with malloc.
	     pre_compile_pattern may use realloc to grow the buffer space.
...
#define ERROR_MSG_MAX_SIZE 200
  ...
  while (p != pend)
	  ...
	  if (many_times_ok)
	    {
	      ...
	      GET_BUFFER_SPACE (3);
	      store_jump (b, greedy ? maybe_finalize_jump : finalize_push,
			  laststart - 3);
	...
	case '[':
	  ...
	  if (b[-1] != (1 << BYTEWIDTH) / BYTEWIDTH)
	    memmove (&b[(int) b[-1]], &b[(1 << BYTEWIDTH) / BYTEWIDTH],
		     2 +
		     EXTRACT_UNSIGNED (&b[(1 << BYTEWIDTH) / BYTEWIDTH]) * 8);
	...
	case '(':
		...
		case '-':
		  for (;;)
			...
			case 'i':
			  if (negative)
			    {
			      if (options & PRE_OPTION_IGNORECASE)
				{
	  ...
	  /* Laststart should point to the start_memory that we are about
	     to push (unless the pattern has PRE_NREGS or more ('s).  */
	  /* obsolete: now PRE_NREGS is just a default register size. */
	...
	case ')':
	  ...
	  if (options != stackp[-1])
	    {
	      if ((options ^ stackp[-1]) & PRE_OPTION_IGNORECASE)
		{
		  BUFPUSH ((options & PRE_OPTION_IGNORECASE) ? casefold_off :
			   casefold_on);
		}
	      BUFPUSH (option_set);
	      BUFPUSH (stackp[-1]);
	  ...
	  if (fixup_alt_jump)
	    {
	      /* Push a dummy failure point at the end of the
	         ...
	         `push_dummy_failure' in `pre_match'.  */
	  ...
	  switch (c = *--stackp)
		...
		*loc = regnum - stackp[-1];
	    ...
	    case '!':
	      ...
	      /* back patch */
	      STORE_NUMBER (bufp->buffer + stackp[-1],
			    b - bufp->buffer - stackp[-1] - 2);
	      stackp--;
	      /* fall through */
	    case '=':
	...
	case '{':
	  ...
	  if (lower_bound >= PRE_DUP_MAX || upper_bound >= PRE_DUP_MAX)
	    FREE_AND_RETURN (stackb, "too big quantifier in {,}");
	  if (upper_bound < 0)
	    upper_bound = PRE_DUP_MAX;
	  ...
	  {			/* If the upper bound is > 1, we need to insert
	    ...
	    /* Initialize lower bound of the `succeed_n', even
	       ...
	       because `pre_compile_fastmap' needs to know.
		...
		GET_BUFFER_SPACE (5);
		store_jump_n (b, greedy ? jump_n : finalize_push_n,
			      laststart + 5, upper_bound - 1);
		b += 5;
	...
	case '\\':
	  ...
	  switch (c)
		...
		memmove (&b[(int) b[-1]], &b[(1 << BYTEWIDTH) / BYTEWIDTH],
			 2 +
			 EXTRACT_UNSIGNED (&b[(1 << BYTEWIDTH) / BYTEWIDTH]) *
			 8);
	      b += b[-1] + 2 + EXTRACT_UNSIGNED (&b[(int) b[-1]]) * 8;
	      break;
	...
	case '#':
	  if (options & PRE_OPTION_EXTENDED)
	    {
	      while (p != pend)
		{
		  PATFETCH (c);
  ...
  if (laststart != b)
    {
      if (*laststart == start_memory)
	laststart += 3;
      if (*laststart == dummy_failure_jump)
	laststart += 3;
  ...
  if (laststart != b)
	...
	laststart += 3;
      if (*laststart == exactn)
	{
	  bufp->options |= PRE_OPTIMIZE_EXACTN;
  ...
  if (bufp->must)
    {
      bufp->must_skip = (int *) malloc ((1 << BYTEWIDTH) * sizeof (int));
...
/* Given a pattern, compute a fastmap from it.  The fastmap records
   ...
   that matches the pattern.  This fastmap is used by pre_search to skip
...
int
pre_search (bufp, string, size, startpos, range, regs)
     struct pre_pattern_buffer *bufp;
     const char *string;
     ...
     struct pre_registers *regs;
{
  ...
  if (bufp->used > 0)
	...
	begbuf_match:
	  if (range > 0)
		...
		{
		  val = pre_match (bufp, string, size, 0, regs);
		  if (val >= 0)
		    return 0;
		  return val;
		}
	...
	case begpos:
	  val = pre_match (bufp, string, size, startpos, regs);
	  if (val >= 0)
	    return startpos;
	  return val;
  ...
  if (bufp->must)
    {
      ...
      pos = bm_search ((unsigned char*)bufp->must + 1, len,
		       (unsigned char*)string + pbeg, (range < 0 ? -range : range),
		       bufp->must_skip, MAY_TRANSLATE ()? translate : 0);

      if (pos == -1)
	return -1;
      if (range > 0 && (bufp->options & PRE_OPTIMIZE_EXACTN))
	{
	  startpos += pos;
	  range -= pos;
	  if (range < 0)
  ...
  for (;;)
	  ...
	  && bufp->can_be_null != 1 && !(anchor && startpos == 0))
	{
      ...
      if (startpos > size)
	return -1;
      if (anchor && size > 0 && startpos == size)
	return -1;
      val = pre_match (bufp, string, size, startpos, regs);
      if (val >= 0)
	return startpos;
	  ...
	  if (anchor && startpos < size &&
	      ...
	      while (range > 0)
		{
		  ...
		  if (*bufp->stclass == charset)
		    {
		      if (!IS_IN_LIST (c, bufp->stclass + 1))
			break;
		    }
		  else
		    {
...
/* The following are used for pre_match, defined below:  */

/* Accessing macros used in pre_match: */

#define IS_ACTIVE(R)  ((R).bits.is_active)
#define MATCHED_SOMETHING(R)  ((R).bits.matched_something)


/* Macros used by pre_match:  */
...
#define PREV_IS_A_LETTER(d) IS_A_LETTER((d)-1)
...
int
pre_match (bufp, string_arg, size, pos, regs)
     struct pre_pattern_buffer *bufp;
     ...
     struct pre_registers *regs;
{
  ...
  unsigned num_regs = bufp->pre_nsub;
  ...
  stacka = PRE_TALLOC (MAX_NUM_FAILURE_ITEMS * NFAILURES, unsigned char *);
  stackb = stacka;
  stackp = stackb;
  stacke = &stackb[MAX_NUM_FAILURE_ITEMS * NFAILURES];

#ifdef DEBUG_REGEX
  fprintf (stderr, "Entering pre_match(%s)\n", string_arg);
#endif
  ...
  for (;;)
    {
      ...
      fprintf (stderr,
	       ...
	       p - (unsigned char *) bufp->buffer, *p);
#endif
      /* End of pattern means we might have succeeded.  */
      if (p == pend)
	{
	  /* If not end of string, try backtracking.  Otherwise done.  */
	  if ((bufp->options & PRE_OPTION_LONGEST) && d != dend)
	    {
	...
	case wordbeg:
	  if (IS_A_LETTER (d)
	      && (AT_STRINGS_BEG (d) || !PREV_IS_A_LETTER (d)))
	    break;
	  goto fail;
...
void
pre_copy_registers (regs1, regs2)
     struct pre_registers *regs1, *regs2;
{
  int i;
